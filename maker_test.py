...
def generate_thumbnail_previews(video_file_path, settings, preview_thumb_size, max_preview_frames, logger):
    logger.info(f"Starting thumbnail preview generation for: {video_file_path} with size {preview_thumb_size}, max frames {max_preview_frames}")
    preview_temp_dir = tempfile.mkdtemp(prefix="movieprint_preview_")
    preview_pil_images = []
    extraction_ok = False # Initialize extraction_ok

    try:
        # Parse Time Settings from the settings object
        ...
        if settings.start_time is not None and start_time_sec is None:
            logger.error(f"Invalid --start_time format for preview: '{settings.start_time}'.")
            return False, []
        if settings.end_time is not None and end_time_sec is None:
            logger.error(f"Invalid --end_time format for preview: '{settings.end_time}'.")
            return False, []
        if start_time_sec is not None and end_time_sec is not None and start_time_sec >= end_time_sec:
            logger.error("Error for preview: --start_time must be less than --end_time.")
            return False, []
        ...
        if settings.extraction_mode == "interval":
            extraction_ok, source_frame_metadata_list = video_processing.extract_frames(
                ...
                output_folder=preview_temp_dir,
                interval_seconds=settings.interval_seconds,
                interval_frames=settings.interval_frames,
                output_format=settings.frame_format if hasattr(settings, 'frame_format') else 'jpg', # Use settings' format or default
            ...
            extraction_ok, source_frame_metadata_list = video_processing.extract_shot_boundary_frames(
                video_path=video_file_path,
                output_folder=preview_temp_dir,
                output_format=settings.frame_format if hasattr(settings, 'frame_format') else 'jpg',
                detector_threshold=settings.shot_threshold,
                start_time_sec=start_time_sec,
                end_time_sec=end_time_sec,
                logger=logger
            )
        else:
            logger.error(f"Unknown extraction mode for preview: {settings.extraction_mode}")
            return False, []

        if not extraction_ok or not source_frame_metadata_list:
            logger.info(f"Preview generation: Frame extraction failed or yielded no frames for {video_file_path}.")
            return False, []

        logger.info(f"Preview generation: Extracted {len(source_frame_metadata_list)} initial frames.")

        # Limit Number of Previews
        if len(source_frame_metadata_list) > max_preview_frames:
            logger.info(f"Preview generation: Limiting {len(source_frame_metadata_list)} frames to {max_preview_frames}.")
            original_count = len(source_frame_metadata_list)
            indices_to_pick = [int(i * (original_count - 1) / (max_preview_frames - 1)) for i in range(max_preview_frames)]
            if max_preview_frames == 1 and original_count > 0: # Edge case
                indices_to_pick = [0]

            selected_frames_metadata = [source_frame_metadata_list[i] for i in sorted(list(set(indices_to_pick)))]

            if len(selected_frames_metadata) > max_preview_frames: # Trim if necessary due to rounding
                selected_frames_metadata = selected_frames_metadata[:max_preview_frames]

            frames_to_keep_paths = {meta['frame_path'] for meta in selected_frames_metadata}

            for item_in_dir in os.listdir(preview_temp_dir):
                item_path = os.path.join(preview_temp_dir, item_in_dir)
                if os.path.isfile(item_path) and item_path not in frames_to_keep_paths:
                    try:
                        os.remove(item_path)
                    except OSError as e:
                        logger.warning(f"Preview generation: Could not remove unselected temp frame {item_path}: {e}")
            source_frame_metadata_list = selected_frames_metadata
            logger.info(f"Preview generation: Selected {len(source_frame_metadata_list)} frames for preview.")


        for meta_item in source_frame_metadata_list:
            frame_path = meta_item['frame_path']
            img = cv2.imread(frame_path)
            if img is None:
                logger.warning(f"Preview generation: Could not read frame {frame_path}. Skipping.")
                continue

            # Resize logic: simple resize for now, could be letterboxed
            resized_img = cv2.resize(img, preview_thumb_size, interpolation=cv2.INTER_AREA)

            # Convert BGR (OpenCV default) to RGB (PIL default)
            rgb_img = cv2.cvtColor(resized_img, cv2.COLOR_BGR2RGB)
            pil_img = Image.fromarray(rgb_img)
            preview_pil_images.append(pil_img)

        logger.info(f"Preview generation: Successfully prepared {len(preview_pil_images)} PIL images for preview.")
        return True, preview_pil_images

    except Exception as e:
        logger.exception(f"Preview generation: An unexpected error occurred: {e}")
        return False, []
    finally:
        if os.path.exists(preview_temp_dir):
            try:
                shutil.rmtree(preview_temp_dir)
                logger.info(f"Preview generation: Cleaned up temporary directory: {preview_temp_dir}")
            except Exception as e:
                logger.error(f"Preview generation: Error cleaning up temp directory {preview_temp_dir}: {e}")
